    ...
    <div class="card-gradient rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          ${typeof tpRenderAmenityChips === 'function' ? tpRenderAmenityChips(h.amenitiesList || h.amenities || ['breakfast','wifi']) : `<div class="text-xs text-slate-500 mt-0.5">${h.amenities || 'завтрак, Wi‑Fi'}</div>`}
...
function openHotelModal(name, destination, checkin, checkout, price, guests) {
  ...
  const _modalHotel = (typeof FEED_HOTELS !== 'undefined' && Array.isArray(FEED_HOTELS))
    ? FEED_HOTELS.find(h => h.name === name && h.city === destination)
    : null;
  const _modalAmenities = _modalHotel?.amenities || ['breakfast', 'wifi'];
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between mb-1">
        <div>
          ...
          ${typeof tpRenderAmenityChips === 'function' ? tpRenderAmenityChips(_modalAmenities) : ''}
        </div>
